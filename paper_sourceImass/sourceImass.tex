...
\begin{abstract}
   ...
   We detect Source I's disk in both continuum and in some lines.
   We resolve the disk around source I and measure its rotation curve.  Because
   ...
   $M_I\gtrsim15$\msun.  These measurements solidify Source I as a genuine
   high-mass protostar system and provide some support for the Source I, BN,
   Source X dynamical decay scenario as an explanation for the explosive
   molecular outflow.
\end{abstract}
...
that self-calibration was ineffective.
...
To emphasize the disk scales, we imaged only baselines $>150$ m (angular scales
$<1.8$\arcsec) in the robust -2 images used for disk fitting and modeling.
...
within a neighboring, apparently signal-free region.
...
Line cubes were produced covering the complete data set to identify lines
...
substantially better fit (Figure \ref{fig:contmodel_residuals_B6}).
...
would remain bound.
Assuming source I's present-day proper motion of 17 \kms reflects its velocity
...
launch).
Material outside $R\gtrsim90$ AU would likely all have become unbound.
...
% -log(2.2/10) / log(7/3)
% -log(2.2/7.3) / log(7/3)
The spectral index from 3 to 7 mm is $\alpha=1.4$, approaching that of an optically
thick blackbody.  {\color{red} NOTE TO COAUTHORS: These numbers have not stayed
...
we show position-velocity diagrams of SiO v=0 J=5-4 in Figure \ref{fig:sio_pvs}.
...
{\color{red} FIGURE (d) IS A PLACEHOLDER - IT IS PRESENTLY IDENTICAL TO (c).}
