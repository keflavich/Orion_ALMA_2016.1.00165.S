...
\begin{abstract}
   We report ALMA long-baseline observations of Orion Source I with resolution 0.03-0.06\arcsec (12-24 AU).
   We detect both continuum and spectral line emission from Source I's disk.
   We resolve the 90 AU diameter disk around Source I and measure its rotation curve.
   ...
   $M_I\gtrsim15$\msun.
   These measurements solidify Source I as a genuine
   high-mass protostar system.
   These measurements support the theory that Source I and the Becklin
...
{\color{red} Band 7 data are not discussed in this work because the data for
...
emission prevents us from obtaining good solutions.
% {\color{red} The phase self-calibrated data shows a significant shift in
% the centroid of Source I and some of the material around it; I am suspicious
...
%shifts source I's centroid a little.}
...
The continuum images have dynamic range in the vicinity of Source I of about 200-400.
These values are reported in Table \ref{tab:image_metadata} and are measured by
taking the ratio of the peak intensity in source I to the standard deviation
within a neighboring, apparently signal-free region (an $r=0.7\arcsec$ circle 1.4\arcesc
to the northwest of Source I).
...
associated with the disk.  Cubes were produced centered on Source I with robust
0.5 and -2 weightings.  These cubes were only cleaned in the
$0.5\arcsec\times0.5$\arcsec region immediately surrounding source I, therefore lines with
...
the median across the 1.8 GHz bandwidth. 
...
continuum disk (Figure \ref{fig:continuum_data_B6}).
...
fitter \citep{Newville2014a}.  The models and their residuals
...
total integrated intensity recovered in the model and the ratio of the compact
central source to the total.

% TODO: investigate flux recovery.  Is it just background emission?
% We recover a total intensity of 280 mJy for the Source I disk
...
{The robust -2 continuum image of Orion Source I at 3 mm (left) and 1.3 mm (right).
The beam is shown
...
Contours are overlaid at $T_B=$50, 100, 150, 200, 300, 400, 500 K.
}
...
the disk continuum peak, the dust column density (and therefore optical depth) drops
rapidly, but the molecular optical depth drops more slowly, so some emission is still
...
the observed lines.  The stacked spectra are shown, with labels,
...
but they are predominantly detected far from the disk midplane.
...
\section{Discussion}
\subsection{The mass of Source I}
...
Even if Source I consists of an equal-mass binary, this mass measurement
confirms that Orion is still a region with ongoing high-mass star formation.
If Source I were below $M<8$ \msun, Orion might instead have been regarded as a
...
but now with source X instead of source n as the third member of the
interaction.}.  Our observed higher mass for source I therefore keeps the
dynamical interaction scenario viable.
...
Source I and BN being launched in nearly opposite directions from their center
...
be dragged in the direction of Source I, and would therefore have a resulting
...
would remain bound.
Assuming source I's present-day proper motion of 11.5 \kms reflects its velocity
...
direction directly opposite source I's ejection direction would have net
velocity relative to source I high enough to escape; the greatest mass
loss would occur if the disk was already in the direction of source I's eventual
launch).
...
that is approximately flat from 1 mm to 3 mm.  Since the source is nearly
...
disk direction in Figure \ref{fig:contmodel_residuals_B6} suggests that we are
seeing the hot inner disk.  As pointed out by \citet{Plambeck2016a}, it is
quite unlikely to be classical free-free emission, since there are no detected
...
one of the expected outcomes of the dynamical scenario \citep{Goddi2011b}, this
detection of the inner region in dust emission provides additional
circumstantial evidence for that scenario.

\section{Conclusions}
We report observations that resolve source I's disk in both continuum
and line emission.  We measure the mass of source I by fitting the 
...
\begin{enumerate}
    \item The central source has mass $M=15-19$ \msun, where the range
        ...
        measurement methods.
    \item The \water $5_{5,0}-6_{4,3}$ line is thermal and kinematically
    ...
    \item Several lines that remain unidentified trace the disk kinematics
        ...
        disk, but are morphologically distinct from both the \water and SiO
    ...
    \item The central emission source in the approximate center of the disk
        ...
        most likely a hot region of the inner disk.
\end{enumerate}
...
implies that the dynamical decay scenario for the Source I - BN - Source X
system is viable.
