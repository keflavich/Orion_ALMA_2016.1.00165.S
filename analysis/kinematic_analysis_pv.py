        ...
        for spw in (0,1,2,3):
            ...
            for linename, linefreq in disk_lines.items():
                ...
                if 'robust' in fn:
                    ...
                    basename = "{0}_{1}_robust{2}_diskpv.fits".format(name,
                                                                      linename,
                                                                      robustnum)
                else:
                ...
                #ax2.set_xlabel("Offset (AU)")
                xticks_pix = ax.get_xticks()
                xticks_arcsec = (ww.sub([1]).wcs_pix2world(ax.get_xticks(), 0) * u.Unit(ww.wcs.cunit[0]) - origin).squeeze()
                xticks_au = (xticks_arcsec * 415*u.pc).to(u.AU, u.dimensionless_angles())
                #ax2.set_xticks(xticks_pix)
                #ax2.set_xticklabels(["{0}".format(int(np.round(x.value))) for x in xticks_au])
                ax.set_xticklabels(["{0:0.1f}".format(x.value) for x in xticks_arcsec])

                # dependent on the positions above being reasonable
                ax.axis(axlims)

                # override that previous junk since we went through the effort of calculating it
                ax.set_xlim(good_limits)
                #ax.set_xbound(good_limits)
