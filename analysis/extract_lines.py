...
for band, cfn in (('B6', b6_hires_cont), ('B7', b7_hires_cont)):
    ...
    conthdu = fits.open(paths.dpath(cfn))
                ...
                for spw in (0,1,2,3):
                    ...
                    if linefreq > fmin and linefreq < fmax:
                        ...
                        for (contband, conthdu) in continua.items():
                            ...
                            cont_levels_Jy = levels.to(u.Jy, beam.jtok_equiv(freqs[contband]))
                            ...
                            mx.FITSFigure.show_contour(conthdu, levels=cont_levels_Jy,
                                                       colors=['r']*10, linewidths=0.75, layer='continuum')
                            mx.FITSFigure.colorbar.set_axis_label_text("$T_B$ [K]")
                            mx.FITSFigure.save(paths.fpath('moments/Orion{1}_{0}_robust{robust}.maskedclarkclean10000_medsub_K_peak_{cont}cont.pdf')
                                               .format(linename, sourcename, robust=robust, cont=contband))
                            ...
                            pl.savefig(paths.fpath('moments/Orion{1}_{0}_robust{robust}.maskedclarkclean10000_medsub_K_peak_offset_{cont}cont.pdf')
                                       .format(linename, sourcename,
                                               robust=robust, cont=contband))
                            if mx.value.max() < 250:
                                line_levels = [50,100,150,200]
                            elif mx.value.max() > 1000:
                                line_levels = [500, 750, 1000, 1250]
                            elif mx.value.max() < 400:
                                line_levels = [150, 250, 350]
                            else:
                                line_levels = [200, 400, 600, 800]
                            con2 = ax.contour(mx.value, levels=line_levels,
                                              zorder=25,
                                              colors=['w']*5, extent=extent, linewidths=0.75)
                            pl.savefig(paths.fpath('moments/Orion{1}_{0}_robust{robust}.maskedclarkclean10000_medsub_K_peak_offset_{cont}cont_contours.pdf')
                                       .format(linename, sourcename,
                                               robust=robust, cont=contband))
                            con.set_alpha(0)
                            ...
                            m0.FITSFigure.save(paths.fpath('moments/Orion{1}_{0}_robust{robust}.maskedclarkclean10000_medsub_K_moment0_{cont}cont.pdf')
                                               .format(linename, sourcename, robust=robust, cont=contband))
                            ...
                            pl.savefig(paths.fpath('moments/Orion{1}_{0}_robust{robust}.maskedclarkclean10000_medsub_K_moment0_offset_{cont}cont.pdf')
                                       .format(linename, sourcename,
                                               robust=robust, cont=contband))
                            ...
                            for coll in con2.collections:
                                coll.set_color('w')
                            pl.savefig(paths.fpath('moments/Orion{1}_{0}_robust{robust}.maskedclarkclean10000_medsub_K_moment0_offset_contours_{cont}cont.pdf')
                                       .format(linename, sourcename,
                                               robust=robust, cont=contband))
