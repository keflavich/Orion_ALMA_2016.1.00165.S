...
min_nchans = 1900
frange = {0: sorted([229173.813e6, 229173.813e6+976.562e3*1920]),
          1: sorted([231840.813e6, 231840.813e6+976.562e3*1920]),
          2: sorted([216156.758e6, 216156.758e6-976.562e3*1920]),
          3: sorted([218855.753e6, 218855.753e6-976.562e3*1920]),
         }
...
def make_spw_cube(spw='spw{0}', spwnum=0, fntemplate='OrionSourceI',
    ...
    fileglob = "OrionSourceI.B6.{0}.lines*{3}".format(spw, fntemplate, fnsuffix,
                                                  filesuffix)
...
if __name__ == "__main__":
    for spw in (0,1,2,3):
        ...
        mxind = get_max_ind('OrionSourceI.B6.spw{0}.lines*fits'.format(spw, ))
        if mxind < min_nchans:
